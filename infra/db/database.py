...
def _apply_security_constraints(conn: sqlite3.Connection) -> None:
    asset_check = "NEW.asset NOT IN ('BTC','LTC','ETH','USDT','SOL')"
    ...
    for table, family_col in (("wallet_addresses", "chain_family"), ("deposits", "chain_family")):
        conn.execute(
            ...
            f"WHEN NOT ((NEW.asset IN ('BTC','LTC') AND NEW.{family_col}=NEW.asset) OR ((NEW.asset IN ('ETH','USDT') AND NEW.{family_col}='ETHEREUM') OR (NEW.asset='SOL' AND NEW.{family_col}='SOLANA'))) "
