...
def init_db(conn: sqlite3.Connection) -> None:
    ...
    deposit_cols = {row["name"] for row in conn.execute("PRAGMA table_info(deposits)").fetchall()}
    ...
    if "credited_amount" not in deposit_cols:
        conn.execute("ALTER TABLE deposits ADD COLUMN credited_amount TEXT")
    if "platform_fee_amount" not in deposit_cols:
        conn.execute("ALTER TABLE deposits ADD COLUMN platform_fee_amount TEXT")
    ...
    if "platform_fee_amount" not in withdrawal_cols:
    ...
    if "submitted" not in table_sql or "confirmed" not in table_sql or "signer_retry" not in table_sql:
        ...
        conn.execute(
            """
            INSERT INTO withdrawals_new(id,user_id,asset,amount,platform_fee_amount,destination_address,status,txid,failure_reason,provider_origin,provider_ref,idempotency_key,external_status,submitted_at,broadcasted_at,last_reconciled_at,tx_metadata_json,created_at)
            SELECT id,user_id,asset,amount,COALESCE(platform_fee_amount,'0'),destination_address,
