...
def main() -> None:
    ...
    try:
        ...
        try:
            upsert_watcher_status(conn, "btc_watcher", success=False, error="; ".join(exc.status.reasons) or str(exc), health="fatal_startup_blocked")
            conn.commit()
        finally:
            conn.close()
    ...
    while True:
        ...
        try:
            ...
            conn.rollback()
            upsert_watcher_status(conn, "btc_watcher", success=False, error=sanitize_runtime_error(exc), health="transient_failure")
            conn.commit()
