...
async def deal_check_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await update.effective_message.reply_text(_render_user_profile(profile_data), reply_markup=keyboard)
        conn.commit()
...
async def deal_amount_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        escrow_service.price_service.get_usd_value(asset, amount)
        escrow_service.create_escrow  # keep lint quiet by reference
...
async def deal_conditions_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await _notify_safe(
            ...
            f"New Deal Request from @{buyer_name} | {amount} {asset} | {conditions} | Created: {created}",
        )
        conn.commit()
...
async def deal_release_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        release_msg = (
            f"@{seller_username} | {view.amount} {view.asset} | {date_row} - {description} | "
...
def main() -> None:
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            DEAL_RATE_SELLER: [CallbackQueryHandler(deal_rate_seller, pattern="^deal_(rate_seller:\d+|finish)$")],
