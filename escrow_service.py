...
class EscrowService:
    ...
    def accept_escrow(self, escrow_id: int, seller_user_id: int) -> EscrowView:
        managed_tx = not bool(getattr(self.conn, "in_transaction", False))
        if managed_tx:
            self.conn.execute("BEGIN IMMEDIATE")
        else:
            self.conn.execute("SAVEPOINT escrow_accept")
        try:
            row = self._escrow(escrow_id)
            if row["status"] != "pending":
                raise ValueError("escrow must be pending")
            if int(row["seller_id"]) != int(seller_user_id):
                raise ValueError("only seller can accept own escrow")
            self.conn.execute("UPDATE escrows SET status='active', updated_at=CURRENT_TIMESTAMP WHERE id=?", (escrow_id,))
            self._event(escrow_id, "accepted", {"actor_user_id": int(seller_user_id)})
            if managed_tx:
                self.conn.commit()
            else:
                self.conn.execute("RELEASE SAVEPOINT escrow_accept")
        except Exception:
            if managed_tx:
                self.conn.rollback()
            else:
                self.conn.execute("ROLLBACK TO SAVEPOINT escrow_accept")
                self.conn.execute("RELEASE SAVEPOINT escrow_accept")
            raise
        row = self._escrow(escrow_id)
    ...
    def cancel_escrow(self, escrow_id: int, actor_user_id: int) -> None:
        ...
        try:
            ...
            if row["status"] == "pending":
                ...
                raise ValueError("active escrow cancellation requires mutual consent — use cancel_escrow_mutual()")
            else:
    ...
    def cancel_escrow_mutual(self, escrow_id: int, requester_user_id: int, responder_user_id: int) -> None:
        """Cancel an active escrow after both parties have agreed via active_cancel_requests.
        Bypasses the active-state guard only after verifying the accepted request record."""
        managed_tx = not bool(getattr(self.conn, "in_transaction", False))
        if managed_tx:
            self.conn.execute("BEGIN IMMEDIATE")
        else:
            self.conn.execute("SAVEPOINT escrow_cancel_mutual")
        try:
            row = self._escrow(escrow_id)
            if row["status"] != "active":
                raise ValueError(f"mutual cancel requires active escrow, got {row['status']!r}")
            participants = {int(row["buyer_id"]), int(row["seller_id"])}
            if int(requester_user_id) not in participants or int(responder_user_id) not in participants:
                raise ValueError("requester and responder must both be escrow participants")
            if int(requester_user_id) == int(responder_user_id):
                raise ValueError("requester and responder must be different participants")
            req = self.conn.execute(
                "SELECT status FROM active_cancel_requests WHERE escrow_id=? AND requester_user_id=? AND responder_user_id=? AND status='open'",
                (escrow_id, int(requester_user_id), int(responder_user_id)),
            ).fetchone()
            if not req:
                raise ValueError("no open mutual cancel request found for this escrow")
            self.wallet_service.cancel_escrow_lock(escrow_id)
            self.conn.execute("UPDATE escrows SET status='cancelled', updated_at=CURRENT_TIMESTAMP WHERE id=?", (escrow_id,))
            self.conn.execute(
                "UPDATE active_cancel_requests SET status='accepted', responded_at=CURRENT_TIMESTAMP WHERE escrow_id=?",
                (escrow_id,),
            )
            self._event(escrow_id, "cancelled", {"requester_user_id": requester_user_id, "responder_user_id": responder_user_id, "method": "mutual"})
            if managed_tx:
                self.conn.commit()
            else:
                self.conn.execute("RELEASE SAVEPOINT escrow_cancel_mutual")
        except Exception:
            if managed_tx:
                self.conn.rollback()
            else:
                self.conn.execute("ROLLBACK TO SAVEPOINT escrow_cancel_mutual")
                self.conn.execute("RELEASE SAVEPOINT escrow_cancel_mutual")
            raise
    ...
    def resolve_dispute(self, escrow_id: int, admin_user_id: int, resolution: str, split_percent: Decimal = Decimal("50")) -> None:
        ...
        if managed_tx:
            ...
            self.conn.execute("SAVEPOINT escrow_resolve_dispute")
        try:
            row = self._escrow(escrow_id)
            ...
            if resolution == "release_seller":
                ...
                tenant = self.tenant_service.get_tenant(int(row["bot_id"]))
                split_fees = self.fee_service.calculate_total_fees(seller_part, tenant.bot_extra_fee_percent)
            ...
            if managed_tx:
                ...
                self.conn.execute("RELEASE SAVEPOINT escrow_resolve_dispute")
        except Exception:
            if managed_tx:
                self.conn.rollback()
            else:
                self.conn.execute("ROLLBACK TO SAVEPOINT escrow_resolve_dispute")
                self.conn.execute("RELEASE SAVEPOINT escrow_resolve_dispute")
            raise
