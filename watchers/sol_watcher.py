...
def run_once(address_user_map: dict[str, int]) -> int:
    ...
    try:
        ...
        adapter = SolRpcAdapter(address_user_map, conn=conn)
        credited = 0
        ...
        for dep in deposits:
            try:
                ...
                LOGGER.exception("failed to ingest SOL deposit event unique_key=%s", dep.unique_key)

        if finalized is not None:
            write_watcher_cursor(conn, "sol_watcher", finalized)
