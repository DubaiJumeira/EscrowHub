...
class WalletService:
    ...
    def get_or_create_deposit_address(self, user_id: int, asset: str) -> DepositRoute:
        ...
        if symbol == "XRP":
            ...
            address, tag = self.hd.derive_btc(user_id).public_address, None
        elif symbol == "LTC":
            address, tag = self.hd.derive_ltc(user_id).public_address, None
        elif symbol in {"ETH", "USDT", "USDC"}:
            address, tag = self.hd.derive_eth(user_id).public_address, None
        elif symbol == "SOL":
            address, tag = self.hd.derive_sol(user_id).public_address, None
        else:
