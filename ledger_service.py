...
class LedgerService:
    ...
    def locked_balance(self, user_id: int, asset: str) -> Decimal:
        ...
        pending_withdrawals = self.conn.execute(
            "SELECT amount, platform_fee_amount FROM withdrawals WHERE user_id=? AND asset=? AND status='pending'",
            (user_id, asset.upper()),
        ).fetchall()
        reserved = Decimal("0")
        for row in locks:
        ...
        for row in pending_withdrawals:
            reserved += Decimal(str(row["amount"] or "0")) + Decimal(str(row["platform_fee_amount"] if "platform_fee_amount" in row.keys() else None or "0"))
        return reserved
