...
- production startup preflight now supports either an approved external derivation/address service or an explicit self-hosted local HD deposit provider for single-node VPS deployments.
...
Self-hosted deposit mode is supported for a single VPS by setting `ADDRESS_PROVIDER=local_hd` (or leaving `ADDRESS_PROVIDER=auto` with `HD_WALLET_SEED_HEX` present). This keeps deterministic address issuance inside the app runtime and is simpler to deploy than an external address service, but it increases hot-runtime key exposure risk compared with a true HSM-backed provider.
...
- External withdrawal providers remain the final go-live dependency for withdrawals. Deposit issuance may use either `ADDRESS_PROVIDER=http` or self-hosted `ADDRESS_PROVIDER=local_hd` on a single VPS.
...
4. If withdrawals are enabled, validate withdrawal provider/signer handshake (`python scripts/staging_smoke_check.py`).
